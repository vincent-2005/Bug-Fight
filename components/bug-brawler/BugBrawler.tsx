...
type Game = { player: { x: number; y: number; hp: number; cash: number; weapon: string; ammo: number; cooldown: number; hurt: number; shield: number; boost: number; speedBurst: number; rapidFire: number; webbed: number; speedMod: number }; enemies: Enemy[]; bullets: Bullet[]; effects: Effect[]; pickups: Pickup[]; wave: number; map: number; clock: number; spawn: number; spawned: number; status: "playing" | "shop" | "won" | "lost"; notice: string; toolbar: ToolbarSlot[]; activeSlot: number; };
...
const roster: Record<EnemyKind, { label: string; hp: number; speed: number; size: number; cash: number; color: string }> = {
  Grubbin: { label: "Grubbin the Tank", hp: 42, speed: .31, size: 20, cash: 18, color: "#ac6f3b" },
  Zippa: { label: "Zippa Waspwing", hp: 14, speed: .88, size: 12, cash: 7, color: "#ffc846" },
  Silk: { label: "Silk Weaver", hp: 24, speed: .52, size: 16, cash: 11, color: "#bc85d9" },
  Formica: { label: "Formica Raider", hp: 36, speed: .39, size: 19, cash: 17, color: "#ee7464" },
  Lumen: { label: "Lumen the Firefly", hp: 20, speed: .64, size: 14, cash: 10, color: "#e8f484" },
  Chomp: { label: "Chomp the Mantis", hp: 24, speed: .78, size: 15, cash: 14, color: "#8bc76c" },
  Roly: { label: "Roly the Pillbug", hp: 50, speed: .27, size: 21, cash: 20, color: "#7e88ad" },
  Skitters: { label: "Skitters", hp: 29, speed: .58, size: 16, cash: 12, color: "#b17242" },
  Rex: { label: "Dungbeetle Rex", hp: 38, speed: .43, size: 18, cash: 16, color: "#435f91" },
  Cicada: { label: "Cicada Screech", hp: 19, speed: .62, size: 14, cash: 8, color: "#61c9c7" },
  Queen: { label: "QUEEN FORMICA, ASCENDED", hp: 360, speed: .45, size: 38, cash: 200, color: "#ff596f" },
};
const shop = [
  { id: "newspaper", name: "Rolled Newspaper +3", cost: 0, stat: "6 DMG · fast melee", desc: "Starter issue. Surprisingly crisp." },
  { id: "zapper", name: "Bug Zapper Lance", cost: 300, stat: "12 DMG · chain shock", desc: "Electric arcs bite nearby pests." },
  { id: "sprayer", name: "Pesticide Sprayer", cost: 325, stat: "9 DMG · poison cone", desc: "A wide, merciless mist." },
  { id: "flame", name: "Anti-Roach Flamethrower", cost: 500, stat: "16 DMG · no revives", desc: "Burns through second chances." },
  { id: "net", name: "Net Cannon", cost: 200, stat: "2 DMG · snare", desc: "Roots targets for a clean follow-up." },
  { id: "grenade", name: "Repellent Grenade", cost: 250, stat: "22 DMG · blast", desc: "A heavy round with a toxic impact." },
  ...
  { id: "launcher", name: "Mire Launcher", cost: 600, stat: "13 DMG · heavy impact", desc: "This one makes the ground tremble." },
...
const ammoFor = (weapon: string) => weapon === "zapper" ? 45 : weapon === "sprayer" ? 70 : weapon === "flame" ? 85 : weapon === "net" ? 20 : weapon === "grenade" ? 16 : weapon === "shotgun" ? 30 : weapon === "horn" ? 12 : weapon === "needle" ? 50 : weapon === "cryo" ? 24 : weapon === "acid" ? 22 : weapon === "pulse" ? 40 : weapon === "whip" ? -1 : weapon === "launcher" ? 22 : weapon === "laser" ? 36 : weapon === "twinblades" ? -1 : -1;
const initial = (): Game => { const progress = loadProgress(); return { player: { x: WORLD_W / 2, y: WORLD_H / 2, hp: 100, cash: 80, weapon: "newspaper", ammo: -1, cooldown: 0, hurt: 0, shield: progress.shieldLevel * 12, boost: 0, speedBurst: 0, rapidFire: 0, webbed: 0, speedMod: progress.speedLevel * .22 }, enemies: [], bullets: [], effects: [], pickups: pickupsFor(0), wave: 1, map: 0, clock: 0, spawn: 20, spawned: 0, status: "playing", notice: "WAVE 1 · CLEAR THE GARDEN", toolbar: [null, null, null, null, null, null], activeSlot: 0 }; };
...
export default function Home() {
  ...
  const canvas = useRef<HTMLCanvasElement>(null); const keys = useRef<Record<string, boolean>>({}); const lookAngle = useRef(-Math.PI / 2); const aim = useRef({ x: WORLD_W / 2, y: WORLD_H / 2, fire: false }); const aimDot = useRef({ x: W / 2, y: H * .6 }); const screenShake = useRef(0); const game = useRef<Game>(initial());
  ...
  useEffect(() => {
    ...
    const frame = () => { const g = game.current, p = g.player, m = maps[g.map]; g.clock++; screenShake.current = Math.max(0, screenShake.current - .75);
      if (g.status === "playing") {
        ...
        const forward = (keys.current.w || keys.current.arrowup ? 1 : 0) - (keys.current.s || keys.current.arrowdown ? 1 : 0), strafe = (keys.current.d || keys.current.arrowright ? 1 : 0) - (keys.current.a || keys.current.arrowleft ? 1 : 0), mag = Math.hypot(forward, strafe) || 1, speed = (3.3 + p.speedMod) * terrainSlow(p.x, p.y, g.map) * (p.speedBurst > 0 ? 1.5 : 1); p.x = Math.max(25, Math.min(WORLD_W - 25, p.x + (Math.cos(facing) * forward - Math.sin(facing) * strafe) / mag * speed)); p.y = Math.max(25, Math.min(WORLD_H - 25, p.y + (Math.sin(facing) * forward + Math.cos(facing) * strafe) / mag * speed)); aim.current.x = p.x + Math.cos(facing) * 600; aim.current.y = p.y + Math.sin(facing) * 600;
        p.cooldown--; p.hurt = Math.max(0, p.hurt - 1); p.boost = Math.max(0, p.boost - 1); p.speedBurst = Math.max(0, p.speedBurst - 1); p.rapidFire = Math.max(0, p.rapidFire - 1); p.webbed = Math.max(0, p.webbed - 1); const weapon = p.weapon === "flame" ? { rate: 3, damage: 16, flame: true, speed: 6.5, life: 18 } : p.weapon === "zapper" ? { rate: 9, damage: 12, flame: false, speed: 10, life: 54 } : p.weapon === "sprayer" ? { rate: 6, damage: 9, flame: false, speed: 10, life: 22 } : p.weapon === "net" ? { rate: 15, damage: 5, flame: false, speed: 8, life: 44 } : p.weapon === "grenade" ? { rate: 20, damage: 22, flame: false, speed: 6, life: 42 } : p.weapon === "shotgun" ? { rate: 17, damage: 7, flame: false, speed: 11, life: 26 } : p.weapon === "horn" ? { rate: 22, damage: 6, flame: false, speed: 14, life: 68 } : p.weapon === "needle" ? { rate: 5, damage: 8, flame: false, speed: 15, life: 82 } : p.weapon === "cryo" ? { rate: 12, damage: 10, flame: false, speed: 11, life: 52 } : p.weapon === "acid" ? { rate: 12, damage: 14, flame: false, speed: 7, life: 42 } : p.weapon === "pulse" ? { rate: 14, damage: 10, flame: false, speed: 12, life: 60 } : p.weapon === "whip" ? { rate: 7, damage: 13, flame: false, speed: 14, life: 15 } : p.weapon === "launcher" ? { rate: 16, damage: 20, flame: false, speed: 7.5, life: 56 } : p.weapon === "laser" ? { rate: 10, damage: 12, flame: false, speed: 20, life: 52 } : p.weapon === "twinblades" ? { rate: 4, damage: 8, flame: false, speed: 12, life: 9 } : { rate: 7, damage: 6, flame: false, speed: 10, life: 54 };
        ...
        g.bullets = g.bullets.filter(b => {
          b.x += b.dx;
          b.y += b.dy;
          const stillInBounds = b.x > -20 && b.x < WORLD_W + 20 && b.y > -20 && b.y < WORLD_H + 20;
          if (--b.life > 0 && stillInBounds) return true;
          if (b.weapon === "grenade") {
            g.effects.push({ x: b.x, y: b.y, life: 28, max: 28, weapon: "grenade" });
            screenShake.current = 14;
          }
          if (b.weapon === "acid") g.effects.push({ x: b.x, y: b.y, life: 22, max: 22, weapon: "acid" });
          return false;
        });
        ...
        if (g.effects.some(effect => effect.weapon === "launcher" && effect.life === 12)) screenShake.current = 10;
        if (g.effects.some(effect => effect.weapon === "grenade" && effect.life === 18)) screenShake.current = 14;
        // Empowered bug abilities: faster dashes, stronger webs, and more frequent ranged pressure.
        g.enemies.forEach(e => {
          const angle = Math.atan2(p.y - e.y, p.x - e.x);
          if (e.kind === "Zippa" && e.cooldown > 38) e.cooldown = 38;
          if (e.kind === "Lumen" && g.clock % 42 === 0) g.bullets.push({ x: e.x, y: e.y, dx: Math.cos(angle) * 9.5, dy: Math.sin(angle) * 9.5, life: 44, damage: 9, flame: false, weapon: "pulse" });
          if (e.kind === "Silk" && g.clock % 58 === 0) p.webbed = Math.max(p.webbed, 80);
          if (e.kind === "Formica" && g.clock % 64 === 0) { e.x += Math.cos(angle) * 58; e.y += Math.sin(angle) * 58; }
        ...
        ctx.clearRect(0, 0, W, H); const shakeOffset = screenShake.current; ctx.save(); if (shakeOffset) ctx.translate((Math.random() - .5) * shakeOffset, (Math.random() - .5) * shakeOffset);
      ...
      if (thirdPerson) {
        ...
        g.effects.forEach(effect => {
          const q = project(effect.x, effect.y); if (!q) return;
          const t = 1 - effect.life / effect.max;
          if (effect.weapon === "acid") {
            const size = (12 + t * 30) * q.scale;
            ctx.save(); ctx.globalAlpha = 1 - t * .45; ctx.fillStyle = "#9dff68"; ctx.shadowColor = "#9dff68"; ctx.shadowBlur = 16;
            ctx.beginPath(); ctx.ellipse(q.x, q.y + size * .18, size, size * .45, 0, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = "#dcff91"; ctx.beginPath(); ctx.arc(q.x - size * .24, q.y - size * .08, Math.max(2, size * .18), 0, Math.PI * 2); ctx.fill();
            ctx.beginPath(); ctx.arc(q.x + size * .25, q.y + size * .09, Math.max(2, size * .13), 0, Math.PI * 2); ctx.fill(); ctx.restore(); return;
          }
          const color = effect.weapon === "zapper" ? "#8cf7ff" : effect.weapon === "sprayer" ? "#9dff68" : effect.weapon === "net" ? "#7de6ff" : effect.weapon === "grenade" ? "#c7ff77" : effect.weapon === "flame" ? "#ff8a42" : "#fff0a2";
          ctx.save(); ctx.globalAlpha = 1 - t; ctx.strokeStyle = color; ctx.lineWidth = 2; ctx.shadowColor = color; ctx.shadowBlur = 10;
          if (effect.weapon === "zapper") { ctx.beginPath(); ctx.moveTo(q.x - 15, q.y + 8); ctx.lineTo(q.x - 5, q.y - 10); ctx.lineTo(q.x + 3, q.y + 5); ctx.lineTo(q.x + 15, q.y - 9); ctx.stroke(); }
          else { ctx.beginPath(); ctx.arc(q.x, q.y, 8 + t * (effect.weapon === "grenade" ? 42 : 24), 0, Math.PI * 2); ctx.stroke(); }
          ctx.restore();
        });
